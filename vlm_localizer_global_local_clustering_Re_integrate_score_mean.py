...
def calc_scores(video_features, sentences, gt, duration, gamma, scores_mean_window_size):
    ...
    #### 비디오 프레임 벡터 스무딩 (로컬)
    smooth_kernel_size = scores_mean_window_size
    smooth_padding = smooth_kernel_size // 2
...
def generate_proposal(video_features, sentences, gt, duration, stride, max_stride, gamma, scores_mean_window_size, nms_thresh=0.3):
    num_frames = video_features.shape[0]
    ground_truth = [round(gt[0] / duration * num_frames, 0), round(gt[1] / duration * num_frames, 0)]
    scores, final_proposals, final_proposals_scores, local_proposals, local_proposals_scores = calc_scores(video_features, sentences, gt, duration, gamma, scores_mean_window_size)
    cum_scores = torch.cumsum(scores, dim=1)[0]
...
def localize(video_feature, duration, query_json, stride, max_stride, gamma, scores_mean_window_size):
    answer = []
    for query in query_json:
        ...
        proposals, scores, pre_proposals, ori_scores, ori_cum_scores, local_proposals, local_proposals_scores, num_frames = generate_proposal(video_feature, query['descriptions'], gt,
                                                                         duration, stride, max_stride, gamma, scores_mean_window_size)
