...
def calc_scores(video_features, sentences, gt, duration, gamma=0.4, alpha=1):
...
def generate_proposal(video_features, sentences, gt, duration, stride, max_stride, gamma=0.5, alpha=1, nms_thresh=0.3):
    num_frames = video_features.shape[0]
    ground_truth = [round(gt[0] / duration * num_frames, 0), round(gt[1] / duration * num_frames, 0)]
    scores, final_proposals, final_proposals_scores, local_proposals, local_proposals_scores = calc_scores(video_features, sentences, gt, duration, gamma, alpha)
    cum_scores = torch.cumsum(scores, dim=1)[0]
...
def localize_best_comb(video_feature, duration, query_json, stride, max_stride, gamma, alpha):
    answer = []
    for query in query_json:
        # import pdb; pdb.set_trace()
        gt = query['gt']
        duration = query['duration']
        proposals, scores, pre_proposals, _, _, _, _, num_frames = generate_proposal(video_feature, query['descriptions'], gt, duration, stride, max_stride, gamma, alpha)
