...
def eval_with_llm(data, feature_path, stride, max_stride_factor, pad_sec=0.0):
    ...
    for vid, ann in pbar:
        ...
        for i in range(len(ann['sentences'])):
            ...
            gt_iou_duration.append({'gt': (gt[1] - gt[0]), 'iou': iou_,'duration': duration})
    ...
    # gt와 iou 값 추출
    gt_values = [d['gt'] for d in gt_iou_duration]
    iou_values = [d['iou'] for d in gt_iou_duration]

    # 점도표 그리기
    plt.scatter(gt_values, iou_values)
    ...
    # 그래프 타이틀 설정
    plt.title('GT vs IoU Scatter Plot')
    plt.savefig('gt_vs_iou_scatter_plot.png')
    plt.close()
    ...
    gt_iou_duration = convert_ndarray_to_list(gt_iou_duration)
    with open('gt_iou_duration.json', 'w') as file:
        json.dump(gt_iou_duration, file)
