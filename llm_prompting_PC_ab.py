...
def select_proposal(inputs, gt, gamma=0.6):
    ...
    mean_iou = 0
    variance_ratio = 0
    correlations = np.zeros_like(weights)
    for j in range(scores.shape[0]):
        iou = calc_iou(proposals, proposals[j])
        gt_iou = calc_iou(proposals, gt)
        variance_iou += np.var(iou)
        mean_iou += np.mean(iou)
        if variance_weights == 0:
            variance_ratio += 0
        else:
            variance_ratio += (variance_iou/variance_weights)
        if np.all(gt_iou == gt_iou[0]) or np.all(weights == weights[0]):
            correlation, p_value = 0, 0
        else:
            correlation, p_value = pearsonr(gt_iou, weights)
        scores[j] += (iou ** gamma * weights).sum()
        correlations[j] = correlation
        # print(iou)
    # print(np.var(inputs[:-1, -1]))
    # import pdb; pdb.set_trace()
    variance_iou /= scores.shape[0]
    mean_iou /= scores.shape[0]
    variance_ratio /= scores.shape[0]
    # print(f'V_iou:{variance_iou:.4f}, V_S:{variance_weights:.4f}')
    idx = np.argsort(-scores)
    return inputs[idx], variance_iou, variance_weights, mean_iou, mean_weights, variance_ratio, correlations.mean()
