...
def eval_with_llm(data, feature_path, stride, max_stride_factor, pad_sec=0.0):
    ...
    for vid, ann in pbar:
        ...
        for i in range(len(ann['sentences'])):
            ...
            proposals = localize(video_feature, duration, query_json, stride, int(video_feature.shape[0] * max_stride_factor), gamma=0.2, cand_num=12, kmeans_k=7, prior=0.5, use_llm_cand_num=5, use_llm=True)
            ...
            if 'query_json' in ann['response'][i]:
                for j in range(len(ann['response'][i]['query_json'][0]['descriptions'])):
                    ...
                    proposals += localize(video_feature, duration, query_json, stride, int(video_feature.shape[0] * max_stride_factor), gamma=0.2, cand_num=12, kmeans_k=7, prior=0.5, use_llm=True)
