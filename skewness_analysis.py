...
def eval_without_llm(data, feature_path, stride, hyperparams, args):
    ...
    q1 = skew_tuples[:total // 4]
    q2 = skew_tuples[total // 4: total // 2]
    q3 = skew_tuples[total // 2: 3 * total // 4]
    ...
    json.dump(filter_data(data, q1), open(f"skewness_analysis/{args.dataset}_top25_data_skew.json", "w"), indent=2)
    json.dump(filter_data(data, q2), open(f"skewness_analysis/{args.dataset}_top25to50_data_skew.json", "w"), indent=2)
    json.dump(filter_data(data, q3), open(f"skewness_analysis/{args.dataset}_bottom50to25_data_skew.json", "w"), indent=2)
