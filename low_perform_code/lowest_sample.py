...
def eval_with_llm(data, feature_path, stride, max_stride_factor, pad_sec=0.0):
    ...
    # Load existing lowest samples
    if os.path.exists('./lowest_sample/lowest_list_200-activityNet.json'):
        with open('./lowest_sample/lowest_list_200-activityNet.json', 'r') as f:
            existing_data = json.load(f)
            # Extracting unique vid-sentence pairs from the existing data
            for item in existing_data:
                vid = list(item[0].keys())[0]  # extract the video id
                sentence = item[0][vid]  # extract the corresponding sentence
                existing_samples.add((vid, sentence))  # store as a tuple for uniqueness
    pdb.set_trace()
    # endregion
    ious = []
    ...
    for vid, ann in pbar:
        ...
        for i in range(len(ann['sentences'])):
            ...
            max_el = None
            if len(lowest) < 300:
                ious_2 = calc_iou(proposals_2, gt)
                max_iou = np.max(ious_2)
                max_index = np.where(ious_2 == max_iou)[0][0]

                obj = [{vid: ann['sentences'][i]}, {'duration': ann['duration']}, {'iou': iou_}, {'relation': relation}, {'gt': gt}, {'proposals': [{'selected_proposal': proposals[:1]}, {'original_proposals': original_query_proposals}, {'sub_proposals': integrated_sub_query_proposals}]}, {'top_candidates':[{'iou': max_iou}, {'proposal': proposals_2[max_index]}]}]
                
                # Check for duplication
                if (vid, ann['sentences'][i]) not in existing_samples:
                    lowest.append(obj)
                    existing_samples.add((vid, ann['sentences'][i]))
            else:
                if max_el == None:
                    max_el = max(lowest, key=lambda x: x[2]['iou'])
                
                if max_el[2]['iou'] > iou_:
                    ious_2 = calc_iou(proposals_2, gt)
                    max_iou = np.max(ious_2)
                    max_index = np.where(ious_2 == max_iou)[0][0]
    ...
    # region
    iou_values = [x[2]['iou'] for x in lowest]
    print('Lowest - mIoU:', sum(iou_values)/len(iou_values))
    ...
    lowest_converted = convert_ndarray_to_list(lowest)
    with open('./lowest_sample/lowest_list_300-activityNet.json', 'w') as file:
        json.dump(lowest_converted, file)
    # endregion
