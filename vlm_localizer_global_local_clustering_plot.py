...
def mean_pool_with_padding(arr, window_size=3):
    # 양 끝값으로 패딩
    padded_arr = np.pad(arr, (window_size//2, window_size//2), mode='edge')
    # mean pooling 처리
    pooled_arr = uniform_filter1d(padded_arr, size=window_size)[window_size//2 : -window_size//2]
    return pooled_arr

def plot_scores(scores, normalized_scores, timestamps, prop1, prop2, filename="scores_plot.png"):
    ...
    # mean pooling 적용
    scores_np_pooled = mean_pool_with_padding(scores_np, window_size=3)
    normalized_scores_np_pooled = mean_pool_with_padding(normalized_scores_np, window_size=3)

    # 그래프 생성
    plt.figure(figsize=(10, 6))
    plt.plot(scores_np_pooled, label="Ours w/o AA", linestyle="-", color='C8', linewidth=2.5)
    plt.plot(normalized_scores_np_pooled, label="Ours", linestyle="-", color="C3", linewidth=2.5)
    plt.xlabel("Frame Index")
    plt.ylabel("Alignment score")
    plt.legend()
    plt.grid(True)
    
    # y축 간격을 자동으로 조정하면서 최대 5개의 눈금으로 제한
...
def generate_proposal(video_features, sentences, gt, duration, stride, max_stride, gamma, kmeans_k, prior, nms_thresh=0.3):
    ...
    for final_proposal in final_proposals_2:
        current_frame = final_proposal[0]
        dynamic_prefix = dynamic_frames[0][current_frame]
        while True:
            if current_frame == 0 or dynamic_frames[0][current_frame - 1] != dynamic_prefix:
...
def localize(video_feature, duration, query_json, stride, max_stride, gamma, cand_num, kmeans_k, prior, use_llm_cand_num=3, use_llm=False):
    ...
    for query in query_json:
        ...
        query['response_2'] = []
        for i in range(len(static_pred_2)):
            query['response_2'].append({
                'start': float(dynamic_pred_2[i]),
                'static_start': float(static_pred_2[i][0]),
                'end': float(static_pred_2[i][1]),
                'confidence': float(scores_2[i])
            })
        answer.append(query)
