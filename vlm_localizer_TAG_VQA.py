...
def generate_proposal_revise(video_features, sentences, stride, hyperparams, kmeans_gpu):
    ...
    return [final_proposals], [final_proposals_scores], scores, num_frames


def localize(video_feature, duration, query_json, stride, hyperparams, kmeans_gpu=False):
    answer = []
    for query in query_json:
        proposals, scores, sim_scores, num_frames = generate_proposal_revise(video_feature, query['descriptions'], stride, hyperparams, kmeans_gpu)
        ...
        for i in range(len(static_pred)):
            _, max_idx = torch.max(sim_scores[0][proposals[0][i][0]:proposals[0][i][1]], dim=0)
            max_idx += proposals[0][i][0]
    ...
    for t in range(cand_num):
        proposals += [[p['response'][t]['static_start'], p['response'][t]['end'], p['response'][t]['num_frames'], p['response'][t]['max_idx']] for p in answer if len(p['response']) > t]  ### only static
