...
def calc_scores(video_features, sentences, gt, duration):
    ...
    #### (글로벌) 클러스터링 결과에 따라 묶음 만들기
     ...
     #### 두 번째 K-means 클러스터링 (selected_video_time_features_local 사용)
    # 첫 번째 클러스터링 구간의 centroid를 초기 중심으로 설정
    kmeans_k = len(centroids_local)
    if num_frames < kmeans_k:
        kmeans_k = 2
    kmeans = KMeans(n_clusters=kmeans_k, init=torch.stack(centroids_local).cpu().numpy(), n_init=1, random_state=42)
    new_kmeans_labels = kmeans.fit_predict(np.array(selected_video_time_features_local.cpu()))
    new_kmeans_labels = torch.tensor(new_kmeans_labels).cuda()
     #### 두 번째 K-means 클러스터링 (selected_video_time_features_local 사용)
    ...
    for i in range(len(local_proposals)):
        for j in range(i + 1, len(local_proposals)):
            start = local_proposals[i][0]
            last = local_proposals[j][0]
