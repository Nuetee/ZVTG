...
def eval_without_llm(data, feature_path, stride, hyperparams, kmeans_gpu):
    ...
    min_k = hyperparams['min_k']

    pbar = tqdm(data.items())

    for vid, ann in pbar:
        duration = ann['duration']
        duration_ratio = duration / (end - start)
        kmeans_k = int(duration_ratio * (max_k - min_k) + min_k)
        video_feature = np.load(os.path.join(feature_path, vid+'.npy'))
        
        for i in range(len(ann['sentences'])):
            gt = ann['timestamps'][i]
            query_json = [{'descriptions': ann['sentences'][i]}]
            proposals = localize(video_feature, duration, query_json, stride, kmeans_k, hyperparams, kmeans_gpu)
...
#         # 현재 범주에 대한 mIoU 및 recall 계산
#         current_mIoU = sum(ious_dict[category]) / len(ious_dict[category])
#         current_recall = recall_dict[category] / len(ious_dict[category])
        
#         pbar.set_postfix({"mIoU": current_mIoU, 'recall': str(current_recall)})
